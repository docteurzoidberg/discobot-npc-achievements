...
async function _saveUserDatabase(userId, db) {
  ...
  try {
    return await fs.promises.writeFile(databaseFile, JSON.stringify(db, null, 2));
  } catch (error) {
...
async function getUserAchievements(userId, showDeleted) {
    ...
    if(!showDeleted)
      return db.achievements.filter(a => !a.dateDeleted);
...
async function uncompleteUserAchievement(userId, achievementId) {
  ...
  if(!db.achievements[found].dateCompleted) {
    throw new Error(`Error updating achievement: ${achievementId} not completed`);
  }
...
module.exports = {
    ...
    addTagUserAchievement, 
    removeTagUserAchievement
};